...
```bash
dradar resume -y --refill --refill-to 5 --max-tasks 20 \
  --quota-tier pro-5x --max-estimated-quota-pct 15
```

`--refill-to` 是持有/运行队列目标，`--max-tasks` 是本次计划从一开始就预留的总题数
硬上限；接近上限时不再补充，让队列自然排空。预计额度上限是第二道保护，会按
`--quota-tier` 换算；缺少可靠估价的题不会在有额度上限时被自动领取。多个本机
